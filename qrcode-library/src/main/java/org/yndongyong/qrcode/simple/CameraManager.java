...
package org.yndongyong.qrcode.simple;
...
public final class CameraManager {
    ...
    public synchronized void openDriver(SurfaceHolder holder) throws IOException {
        ...
        if (supportedPreviewSizes.get(0).width > supportedPreviewSizes.get(supportedPreviewSizes.size() - 1).width) {
            previewSize = supportedPreviewSizes.get(0);// 从List取出Size
        } else {
            previewSize = supportedPreviewSizes.get(supportedPreviewSizes.size() - 1);// 从List取出Size
        }
        ...
        if (supportedPictureSizes.get(0).width > supportedPictureSizes.get(supportedPictureSizes.size() - 1).width) {
            pictureSize = supportedPictureSizes.get(0);// 从List取出Size
        } else {
            pictureSize = supportedPictureSizes.get(supportedPictureSizes.size() - 1);// 从List取出Size
        }
